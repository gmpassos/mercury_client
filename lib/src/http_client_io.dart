...
class HttpClientRequesterIO extends HttpClientRequester {
  ...
  Future<io.HttpClientRequest> _request(HttpClient client, HttpRequest request, Uri uri) async {
    ...
    switch (method) {
      ...
      default: throw UnsupportedError("Can't handle method: $method") ;
    }
  }

  Future<io.HttpClientRequest> _requestGET(HttpClient client, HttpRequest request, Uri uri) async {
    var req = await io.HttpClient().get(uri.host, uri.port, toPathWithQuery(uri, request)) ;
    _putRequestHeaders(request, req);
    return req ;
  }

  Future<io.HttpClientRequest> _requestPOST(HttpClient client, HttpRequest request, Uri uri) async {
    var req = await io.HttpClient().post(uri.host, uri.port, toPathWithQuery(uri, request)) ;
    _putRequestHeaders(request, req);
    _putSendData(request, req);
    return req ;
  }

  Future<io.HttpClientRequest> _requestPUT(HttpClient client, HttpRequest request, Uri uri) async {
    var req = await io.HttpClient().put(uri.host, uri.port, toPathWithQuery(uri, request)) ;
    _putRequestHeaders(request, req);
    _putSendData(request, req);
    return req ;
  }

  Future<io.HttpClientRequest> _requestPATCH(HttpClient client, HttpRequest request, Uri uri) async {
    var req = await io.HttpClient().patch(uri.host, uri.port, toPathWithQuery(uri, request)) ;
    _putRequestHeaders(request, req);
    _putSendData(request, req);
    return req ;
  }

  Future<io.HttpClientRequest> _requestDELETE(HttpClient client, HttpRequest request, Uri uri) async {
    var req = await io.HttpClient().delete(uri.host, uri.port, toPathWithQuery(uri, request)) ;
    _putRequestHeaders(request, req);
    _putSendData(request, req);
    return req ;
  }

  Future<io.HttpClientRequest> _requestHEAD(HttpClient client, HttpRequest request, Uri uri) async {
    var req = await io.HttpClient().head(uri.host, uri.port, toPathWithQuery(uri, request)) ;
    _putRequestHeaders(request, req);
    return req ;
  }
  ...
  String toPathWithQuery(Uri uri, HttpRequest request) {
    ...
    if ( queryParameters != null && queryParameters.isEmpty ) queryParameters = null ;
